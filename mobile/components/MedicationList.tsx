...
import { useGetMedications } from '../hooks/api/useMedications';
...
export default function MedicationList({ encounterId }: MedicationListProps) {
  ...
  const { data, isLoading: loading, refetch, isFetching } = useGetMedications(encounterId);
  const defaultData = data || { prescriptions: [], administrations: [] };
  ...
  const getActiveItems = () => {
    if (!defaultData.prescriptions) return [];
    return defaultData.prescriptions.flatMap((p: any) => 
...
const HistoryMedCard = memo(({ item }: { item: any }) => (
    <View style={styles.historyItem}>
      <View style={styles.timelineLeft}>
         <View style={[styles.timelineDot, item.status === 'GIVEN' ? {backgroundColor: theme.colors.success} : item.status === 'NOT_GIVEN' ? {backgroundColor: theme.colors.error} : {backgroundColor: theme.colors.warning}]} />
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={activeTab === 'active' ? getActiveItems() : defaultData.administrations}
        ...
        ListEmptyComponent={
            <View style={styles.emptyContainer}>
                <Ionicons name="ban-outline" size={40} color={theme.colors.border} />
...
const styles = StyleSheet.create({
  ...
  tabsBackground: { flexDirection: 'row', backgroundColor: theme.colors.border, borderRadius: 12, padding: 4 },
  tab: { flex: 1, paddingVertical: 8, alignItems: 'center', borderRadius: 10 },
  activeTab: { backgroundColor: theme.colors.surface, ...theme.shadows.small },
  tabText: { fontWeight: '600', color: theme.colors.textLight, fontSize: 13 },
  activeTabText: { color: theme.colors.text },

  listContent: { paddingHorizontal: 16, paddingBottom: 20 },

  // Card
  card: { backgroundColor: theme.colors.surface, borderRadius: 16, padding: 16, marginBottom: 16, ...theme.shadows.small, borderWidth: 1, borderColor: theme.colors.border },
  ...
  genericName: { fontSize: 13, color: theme.colors.textLight, fontStyle: 'italic', marginTop: 2 },
  
  divider: { height: 1, backgroundColor: theme.colors.border, marginBottom: 12 },
  
  detailsGrid: { flexDirection: 'row', marginBottom: 12 },
  detailItem: { flex: 1 },
  detailLabel: { fontSize: 11, color: theme.colors.textMuted, textTransform: 'uppercase', marginBottom: 4 },
  detailValue: { fontSize: 14, fontWeight: '600', color: theme.colors.text },

  notes: { fontSize: 13, color: theme.colors.textLight, backgroundColor: theme.colors.background, padding: 10, borderRadius: 8, marginBottom: 12, overflow: 'hidden' },

  footer: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 },
  doctorInfo: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  doctorName: { fontSize: 12, color: theme.colors.textLight, fontWeight: '500' },
  
  administerBtn: { backgroundColor: theme.colors.primary, paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20 },
  btnText: { color: theme.colors.surface, fontWeight: '600', fontSize: 12 },
  ...
  historyContent: { flex: 1, backgroundColor: theme.colors.surface, borderRadius: 12, padding: 14, marginBottom: 16, borderWidth: 1, borderColor: theme.colors.border },
  historyHeader: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6, alignItems: 'center' },
  timestamp: { fontSize: 13, fontWeight: '600', color: theme.colors.text },
  ...
  textYellow: { color: theme.colors.warning },

  performer: { fontSize: 12, color: theme.colors.textLight, marginBottom: 2 },
  historyNotes: { fontSize: 12, color: theme.colors.textLight, fontStyle: 'italic', marginTop: 4 },

  emptyContainer: { alignItems: 'center', marginTop: 40 },
  emptyText: { marginTop: 8, color: theme.colors.textMuted, fontSize: 14 }
});
