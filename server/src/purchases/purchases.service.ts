...
export class PurchasesService {
  ...
  //           const priceIn = Number(line.unitPrice);

  //           // 🛡️ استخدام الحجز الصارم (Locking) للمنتج أثناء تحديث التكلفة
  ...
  //             // (الكمية الحالية * تكلفتها) + (الكمية الجديدة * سعرها) / إجمالي الكمية
  //             let newWac = priceIn;
  //             if (currentStock > 0) {
  //               newWac =
  //                 (currentStock * currentCost + qtyIn * priceIn) /
  //                 (currentStock + qtyIn);
  //             }

  //             // تحديث المنتج بالرصيد والتكلفة الجديدة
  //             await tx.product.update({
  //               where: { id: line.productId },
  //               data: {
  //                 stockOnHand: { increment: qtyIn },
  //                 costPrice: newWac, // ✅ التكلفة أصبحت دقيقة محاسبياً
  //               },
  //             });

  //             // ... (بقية منطق تحديث ProductStock و StockTransaction كما هو)
  //             // تأكد من استخدام newWac أو priceIn حسب طبيعة التقرير
  //           }
  //         }
  //       }
  //     }

  //     // تحديث حالة الفاتورة وإصدار القيد المحاسبي
