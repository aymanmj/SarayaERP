...
// import { PrismaService } from 'src/prisma/prisma.service';
// import { ChargeSource, OrderStatus, OrderType, LabResultStatus } from '@prisma/client';
...
//   /**
//    * إرجاع كل طلبات التحاليل المرتبطة بـ Encounter معيّن
//    */
//   async listForEncounter(encounterId: number) {
...
//       throw new NotFoundException('الحالة غير موجودة أو لا تنتمي لنفس المستشفى');
//     }

//     // نجيب كل التحاليل المطلوبة دفعة واحدة
//     const labTests = await this.prisma.labTest.findMany({
//       where: {
//         id: { in: testIds },
//         hospitalId,
...
//     if (labTests.length !== testIds.length) {
//       throw new BadRequestException('بعض التحاليل غير صالحة أو غير مفعّلة');
//     }

//     // نشتغل داخل Transaction
...
//         // 2) إنشاء LabOrder مرتبط بهذا الـ Order + LabTest
//         const labOrder = await tx.labOrder.create({
//           data: {
//             orderId: order.id,
//             testId: test.id,
...
//         // 3) إنشاء Charge تلقائي لو التحليل مربوط بخدمة تسعير
//         if (test.serviceItemId && test.serviceItem) {
...
//               serviceItemId: test.serviceItemId,
//               sourceType: ChargeSource.LAB_ORDER,
//               sourceId: order.id,   // نربطه بالـ Order
//               quantity: 1,
//               unitPrice: test.serviceItem.defaultPrice,
//               totalAmount: test.serviceItem.defaultPrice,
//             },
//           });
//         }
//       }
